...
#if 1
  auto operator<=>(const task_t& rhs) const { return id <=> rhs.id; }
#else
...
struct group_t
{
  // str_t color; // TODO: generate group unique hex color, can be overridden by
  // the user
  ...
  str_t            gname{ "group_" +
               std::to_string(gid) }; // TODO: can be overridden by the user
  s32              gid{ getID() };
#if 1
  auto operator<=>(const group_t& rhs) const { return gid <=> rhs.gid; }
#else
