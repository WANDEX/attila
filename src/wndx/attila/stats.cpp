...
stats_t calculate_stats(vtasks_t const& vtt)
{
  ...
  std::size_t const nrecords{ vtt.size() };

  std::size_t sec{ 0 }; // total spent on task in seconds
  for (auto const& t : vtt) {
...
stats_human_t calculate_stats_human(stats_t const& t)
{
  // convert size_t seconds into HH:MM spent time string
  auto hm = [&](std::size_t const sec) -> str_t {
                                             ...
                                             str_t const&    mulstr)
{
  ...
  for (std::size_t i = 0; i < v.size(); ++i) {
    ...
    for (std::size_t j = i; j < v.size(); ++j) {
      ...
      if (same_text) {
        // NOTE: set -> since we do not want to insert the same thing more than
        // once
        subt_t.insert(v[j]);
        v.erase(v.begin() +
                static_cast<long>(
                    j--)); // remove by index & decrement index afterwards
      }
    }
    v.at(i).subt_t.insert(subt_t.begin(),
                          subt_t.end()); // put set of sub tasks as child's
  }
  ...
  for (auto& main_task : v) {
    ...
    std::time_t sec{ 0 };
    for (auto const& sub_task : main_task.subt_t) {
      sec += sub_task.hm.diff;
    }

    auto const last = main_task.subt_t.rbegin();
    ...
    // if first & last sub-task date differ -> only date strings without time:
    // fr -> to
    std::ostringstream out;
    if (main_task.hm.date_fr == main_task.hm.date_to) {
      out << "*M  (" << main_task.hm.date_fr << ") " << main_task.hm.time_fr
          << " > " << main_task.hm.time_to;
    } else {
...
sgroups_t auto_proj_groups(vtasks_t const& vtt)
{
  ...
  for (auto& task : vtt) {
    ...
    str_t const project_name{
      task.tproj[0]
    }; // if more than one -> first task project

    // auto-associate tasks with the same project name to the same auto group
    // do not create new group if group with project name already exist in
    // groups -> simply insert task to the group
    if (utproj_names.find(project_name) != utproj_names.end())
    {                         // exist -> reuse already present group
      bool found = false;
      ...
      if (!found) {
        try { // XXX
          throw std::runtime_error(
              "Finished iterating over groups -> project_name, was not found!");
      ...
      utproj_names.insert(
          project_name); // since this is a set duplicates wont be added
    }
  }
