...
inline static char const* datef{ "%Y-%m-%d" }; // date format
inline static str_t const date_rs{
  R"((\d{4}.\d\d.\d\d|\d\d.\d\d.\d{4}|\d\d.\d\d.\d\d))"
};
inline static str_t const time_rs{ R"(.*(\d\d:\d\d).*(\d\d:\d\d))" };
inline static regex const dts_re{ str::date_rs +
                                  str::time_rs };  // date + time span regex
inline static regex const dts_txt_re{ "(^.*" + str::time_rs +
                                      ") (.*$)" }; // + task text
